...
- `CoursePlan`：课程计划。
- `TrainingRecord`：训练记录。
- `ExamSite`：考场主数据（名称、科目类型、可考车型、地址、区域、默认容量）。
- `ExamSiteSchedule`：考场日程（按日期、科目维护容量与余位）。
- `ExamSiteCapacitySnapshot`：考场容量快照（用于预约趋势统计）。
...
- 报考：报考分页、审核、考场容量校验（默认容量 + 按日期/科目日程余位校验）。
- 真实考场：支持 Excel 导入真实考场主数据，按日期/科目/区域实时查询容量、剩余名额与预约趋势。
...
| 待完善任务         | 说明                                                         |
| ------------------ | ------------------------------------------------------------ |
| 实时查询考场       | 支持按日期、科目、区域实时查看考场容量、剩余名额、预约趋势。 |
...
| 消息通知中心       | 新增报考审核结果、课程计划变更、考场容量预警等站内通知。     |
